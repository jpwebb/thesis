...
function device = getDevice(DeviceInfo)
...
elseif length(device) ~= 1
    clc;
    fprintf('Error, please select one device only!\n\n');
    error_flag = true;
end
...
if ~error_flag
    ...
    for i = 1 : length(device)
        if ~any(contains(string({DeviceInfo.ID}'), string(device(i))))
            if first_time
                clc;
                first_time = false;
            end
            fprintf(['Error, device ID ', num2str(device(i)), ' unavailable!\n']);
            error_flag = true;
        end
    end
    if error_flag
...
if ~error_flag && (length(device) - length(unique(device)) ~= 0)
    clc;
